...
%%
%pre-processing step
[Z_Xtest, Z_Xtrain] = PreProcess.binariZe(Xtest, Xtrain);
...
for i=1:numTest       % for each data-line in test set...    
    for n=1:numTrain  %calculate eclu. dst to each data-line in training set
        dist_matrix(i,n) = sum(abs((Z_Xtest(i,:) - Z_Xtrain(n,:))));      
    end
end   
...
xlabel('k');
ylabel('Error rates (%)');
title('Q4(binariZed): Plot of error rates vs. k'); 
